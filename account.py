...
class Account():
    ...
    def Create_Transaction(self, cataLog: str, fluctuation: str, amount: int, time: str = "YYYY-MM-DD", note: str = "", ID: int = 0) -> bool:
        if amount > 0:
            import trans
            newTransaction = trans.Transaction(ID=self.__CreateID() if ID == 0 else ID, amount=amount, cataLog=cataLog.strip(), fluctuation=fluctuation, note=note.strip(), time=time.strip())
